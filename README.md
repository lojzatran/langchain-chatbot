...
4. Start observability infra (optional):
...
| **Logs exporter**    | debug stdout - TBD                                                      |
...
There are currently 3 different sources of metrics:

1. The metrics that are coming from Prometheus itself. These metrics are prefixed with `prometheus_`.
1. The metrics that are coming from OpenTelemetry collector. These metrics are prefixed with `otel_`.
1. The metrics are created from spans instrumented by Next.js. These metrics are prefixed with `langchain_chatbot_`. There are 4 dimensions for every metric:
